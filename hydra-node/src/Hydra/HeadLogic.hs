...
update env ledger st ev = case (st, ev) of
  ...
  (Open openState, NetworkEvent _ (ReqSn otherParty sn txIds)) ->
  ...
  (_, NetworkEvent _ (Connected nodeId)) ->
    Effects [ClientEffect $ PeerConnected{peer = nodeId}]
  (_, NetworkEvent _ (Disconnected nodeId)) ->
    Effects [ClientEffect $ PeerDisconnected{peer = nodeId}]
